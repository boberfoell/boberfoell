...
RoundBase=2; //[0:minimum,1:rounded,2:rectangular]
...
module RiseText(textstr, textsize, direction="up") 
{
    rotate([0,-90,0])
    {
        ...
        else if(RoundBase==1)    // rounded letter base
        {
            translate([(direction=="up"?-1:0)*baseheight, text_excenter_factor*textsize, 0])
                rotate([0,(direction=="up"?1:-1)*90,0])scale([1,(direction=="up"?1:-1),1])
                    linear_extrude(height=baseheight, twist=0, slices=1, $fn=32, convexity = 32) 
        ...
        else  // if RoundBase==2 -> rounded rectangle base
        {
          $fn=32;
          minkowski() {
            translate([(direction=="up"?-1:0)*baseheight, text_excenter_factor*textsize, 0])
                rotate([0,(direction=="up"?1:-1)*90,0])scale([1,(direction=="up"?1:-1),1])
                    linear_extrude(height=baseheight, twist=0, slices=1, convexity = 32) 
                        offset(r=base_radius_add)
                          {  
                            square([0.75*textsize*(len(textstring)),1.25*textsize], center=true);
                          }
            sphere(baseheight);
          }
        }
